            ...
            if let saveContent = saveResult.first, let code = saveContent.code {
                ...
                if let currency = try container.viewContext.fetch(currencyRequest).first {
                    ...
                    DispatchQueue.main.async {
                        navigationController.pushViewController(ExchangeRateCalculatorViewController(viewModel: ExchangeRateCalculatorViewModel(currencyModel: currencyModel)), animated: true)
                    }
                } else {
                    print("Currency 값 없음...")
                    window.rootViewController = navigationController
